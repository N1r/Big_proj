...
from batch.utils.video_processor import process_video, process_video_simple
...
def process_batch(excel_path='batch/tasks_setting.xlsx'):
    ...
    for index, row in df.iterrows():
        if pd.isna(row['Status']) or 'Error' in str(row['Status']):
            ...
            try:
                ...
                status, error_step, error_message = process_video_simple(video_file, dubbing, is_retry)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Process video tasks from Excel file.")
    parser.add_argument('--excel', type=str, default='batch/tasks_setting.xlsx', help='Path to Excel task file')
    args = parser.parse_args()
    process_batch(excel_path=args.excel)
